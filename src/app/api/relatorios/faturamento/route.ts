  ...
  try {
    ...
    if (formato === "pdf") {
      ...
      const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
        ...
        // Título
        doc.fontSize(18).fillColor('#1a202c').text('Relatório de Faturamento', { align: 'center', underline: true });
        doc.moveDown(0.5);
        doc.fontSize(10).fillColor('#333').text(`Período: ${new Date(data_inicio).toLocaleDateString()} a ${new Date(data_fim).toLocaleDateString()}`);
        ...
        // Total em destaque
        doc.fontSize(10).fillColor('#1a202c').font('Helvetica-Bold').text(`Total Faturado: R$ ${totalFaturado.toFixed(2)}`, startX, y, {
