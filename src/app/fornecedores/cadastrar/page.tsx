  ...
  const onSubmit: SubmitHandler<FornecedorFormData> = async (data) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Falha ao cadastrar fornecedor");
      }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div className="sm:col-span-3">
              <Field id="complemento" label="Complemento" error={errors.complemento?.message}>
                <input type="text" maxLength={100} id="complemento" {...register("complemento")} className={inputClassName} placeholder="Sala 201, Bloco B" />
              </Field>
            </div>
          </div>
        ...
        <div className="space-y-4 pt-2">
          <Field id="observacoes" label="Observações" error={errors.observacoes?.message}>
            <textarea
              ...
              rows={4}
              placeholder="Anotações adicionais sobre o fornecedor..."
              className={inputClassName}
            />
          </Field>
        </div>
      ...
      <div className="bg-white/80 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-4 sm:p-8 transition-colors">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Preencha os dados abaixo para adicionar um novo fornecedor ao sistema.
          </p>
        </div>
